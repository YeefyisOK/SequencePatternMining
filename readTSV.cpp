...
int main()
{
	...
	const string out_path = out_dir + "\\FourTypePercent9.csv";
	...
	for (int k = 0; k < names.size(); k++) {  
		...
		while (fgets(original_data, sizeof(original_data), fp))//original_data != ""
		{
			...
			if (start_flag == 0) {
				//读取第一行不计算duration
				recordingTime = stoi(line[0]);
				start_flag = 1;
			}
			else if(start_flag==1){
				if (line[54] != last_eye_movement_type) {
					int duration = stoi(line[0]) - recordingTime;
					recordingTime = stoi(line[0]);
			...
			lastLineRecordingTime = stoi(line[0]);
			if (feof(fp))
				break;
		}
